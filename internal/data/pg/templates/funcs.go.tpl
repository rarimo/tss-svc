...
import (
	...
	"gitlab.com/rarify-protocol/xo/templates/gotpl"
...
func (f *Funcs) funcfn(name string, context bool, v interface{}) string {
	...
	case gotpl.Index:
	    qName = x.Table.GoName
		// params
		p = append(p, f.params(x.Fields, true))
		p = append(p, "isForUpdate bool")
		// returns
		rt := "*data." + x.Table.GoName
		if !x.IsUnique {
			rt = "[]data." + x.Table.GoName
		}
		r = append(r, rt)
	...
	r = append(r, "error")
	return fmt.Sprintf("func (q %sQ) %s(%s) (%s)", qName, name, strings.Join(p, ", "), strings.Join(r, ", "))
}
...
func (f *Funcs) recv(name string, context bool, t gotpl.Table, v interface{}) string {
	...
	short := f.short(t)
	p = append(p, fmt.Sprintf("%s *data.%s", short, t.GoName))
	switch x := v.(type) {
	case gotpl.ForeignKey:
		r = append(r, "*"+x.RefTable)
	}
	r = append(r, "error")
	return fmt.Sprintf("func (q %sQ) %s(%s) (%s)", t.GoName, name, strings.Join(p, ", "), strings.Join(r, ", "))
}
...
// db generates a db.<name>Ctx(ctx, sqlstr, ...)
func (f *Funcs) db(name string, destName string, v ...interface{}) string {
	// params
	var p []interface{}
	if f.contextfn() {
		name += "Context"
	...
	p = append(p, "sqlstr")
	return fmt.Sprintf("q.db.%s(%s)", name, f.names("", append(p, v...)...))
}

// db_prefix generates a db.<name>Ctx(ctx, sqlstr, <prefix>.param, ...).
//
// Will skip the specific parameters based on the type provided.
func (f *Funcs) db_prefix(name, destName string, skip bool, vs ...interface{}) string {
	var prefix string
	...
	return f.db(name, destName, params...)
}

// db_update generates a db.<name>Ctx(ctx, sqlstr, regularparams,
// primaryparams)
func (f *Funcs) db_update(name string, v interface{}) string {
	...
	case gotpl.Table:
		...
		ignore = append(ignore, "CreatedAt")
...
// db_named generates a db.<name>Ctx(ctx, sql.Named(name, res)...)
...
func (f *Funcs) colnames(fields []gotpl.Field) string {
    var names []string
    for _, field := range fields {
        names = append(names, f.colname(field))
    }
